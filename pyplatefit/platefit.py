...
class Platefit:
    ...
    def contfit(self, sp, z, vdisp):
        ...
        vacwl_orig = airtovac(vacwl_orig)
        ...
        for isz in range(nsz):
            ...
            if contcoefs[0] < -90:
                ...
                self.settings['best_sz'] = -99.0
                self.settings['best_ebv'] = -99.0
                self.settings['best_modelChi'] = 99999.99
        ...
        idxbest_sz = np.argmin(np.abs(np.array(self.settings['szval']) - best_szval))
    
    
        
        # save the fitted values into the settings
        self.settings['best_spectrum'] = best_continuum
        self.settings['best_sz'] = best_szval
        self.settings['best_ebv'] = best_ebv
        self.settings['best_modelChi'] = best_modelChi
        self.settings['best_ages'] = self.settings['ssp_ages'][np.array(np.where(best_contCoefs[1:]>0)).squeeze()]
        self.settings['best_weights'] = best_contCoefs[1:][np.array(np.where(best_contCoefs[1:]>0)).squeeze()]
    
        # The best spectrum/ages/weight parameters in a given gas- and stellar-metallicity bin
        self.settings['model_spectrum'] = continuum
        self.settings['model_library'] = model_library
        self.settings['model_ebv'] = ebv
        self.settings['model_weights'] = model_contcoeff
        self.settings['model_chi'] = model_chi
        self.settings['model_ebv'] = ebv
        
        self.settings['ok_fit'] = ok_fit
            
        
        self.logger.debug('best model chi squared value: %s', best_modelChi)
        self.logger.debug('all model chi squared values = %s', model_chi)
        self.logger.debug('best E(B-V) = %s', best_contCoefs[0])
    
        self.logger.debug('best model coefs = %s', np.stack((best_contCoefs[1:], self.settings['ssp_ages']/1.0E6), axis=-1))
        self.logger.debug('best stellar dZ: %s', best_szval)
    ...
    def model_combine(self, x, a, settings_nnls, good_data=None, ssp_ages=None, individual=False, correct=None):
        """
            Jan 15, 2019, Madusha Gunawardhana (gunawardhana@strw.leidenuniv.nl)
